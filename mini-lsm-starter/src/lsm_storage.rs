...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let mut l0_iters: Vec<Box<SsTableIterator>> =
        ...
        for (_, level_sst_ids) in &snapshot.levels {
            ...
            let level_iter = SstConcatIterator::create_and_seek_to_key(level_ssts, key_slice)?;
    ...
    pub fn scan(
        ...
        for (_, level_sst_ids) in &snapshot.levels {
            ...
            let level_iter = match lower {
                ...
                Bound::Excluded(key) => {
                    let key_slice = KeySlice::from_slice(key);
                    let mut iter =
                        SstConcatIterator::create_and_seek_to_key(level_ssts, key_slice)?;
                    if iter.is_valid() && iter.key() == key_slice {
                        iter.next()?;
                    }
                    iter
        ...
        let iter = TwoMergeIterator::create(mem_table_iter, l0_iter)?;
